	  ...
	  <?php
			if(isset($_SESSION["user_id"])  && !empty($_SESSION['user_type'])){
				if($_SESSION['user_type'] == "seller"){		
					...
					$result = mysqli_query($ob->con,"SELECT * FROM users WHERE user_id='$userID'");
	...
	<div class="card">
	  ...
	  <div class="card-body">
			...
			<div  style="display:none" class="alert alert-success" id="deleteSuccessDiv">
				<strong>Success!</strong> Product Archived successfully.
			</div>
					...
					<tbody id="tbodyDataTable">
						 <?php
							...
							while($row = mysqli_fetch_array($result))
							{
								...
								$result2 = mysqli_query($ob->con,"SELECT * FROM product_category WHERE prod_cat_id='$prod_cat_id'");
								$prod_cat_name = "";
							...
							echo "<td>";
							echo "<button id='deleteTableBtn' class='btn btn-danger btn-sm btn-block'><i class='fa fa-trash' > Archive</i></button>";
							if($row['is_availabe'] == 'yes')
								echo "<span class='badge badge-success'>SOLD</span></i></button>";
							else
		...
		<div class="modal-content">
			<form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate >
			  ...
			  <div class="modal-body">
				...
				<div class="form-row">
					<div class="col-md-6 my-1">
						<label for="exampleFormControlTextarea1">Location<span style="color:red;">*</span></label>
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
			<form method="POST" >
			  ...
			  <div class="modal-body">
				<H4>Are you sure want to Archive product?</H4>
				<p class="text-warning">this action can`t undo.</p>
				<input type="hidden" id="prod_id_hidden" name="prod_id_hidden" >
			  </div>
			  <div class="modal-footer btn-group" role="group">
				<button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
				<input type="submit" id="deleteProduct" name="deleteProduct" class="btn btn-danger btn-sm" value="Archive Product"/>
			  </div>
		  </form>
		</div>
	  </div>
...
<!-- ======= Footer ======= -->
  ...
  <?php
			...
			if(isset($_POST['addProduct'])){
				...
				$sql = "INSERT INTO product (user_id,prod_cat_id,product_name,sizeOrWeight,qty,price,start_date,end_date,address,description) 
					VALUES ('$userID','$prod_cat_id','$product_name','$sizeOrWeight','$qty','$price','$start_date','$end_date','$address','$description')";
				
				if ($ob->con->query($sql) === TRUE) {
						$last_id = $ob->con->insert_id;
