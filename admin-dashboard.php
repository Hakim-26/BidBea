			...
			<table id="dataTable" class="table table-striped table-bordered" style="width:100%; text-align: center;">
					...
					<tbody id="tbodyDataTable">
						 <?php
							...
							echo "<td> <a   href='#' id='govt_id'>" . $row['govt_id'] . "</a></td>";
	...
	<div class="modal fade" id="imageModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
			<form method="POST" >
			  <div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Govt ID</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				  <span aria-hidden="true">&times;</span>
				</button>
			  </div>
			  <div class="modal-body">
				<img  id ="govtImg" src="" style="width:460px; height:200px">
			  </div>
  ...
  <?php
			...
			if(isset($_POST['addProduct'])){
				...
				if ($ob->con->query($sql) === TRUE) {
					...
					echo "<script type='text/javascript'>
							$(document).ready(function(){ 
								$('#warningDivSpan').text('Error: ' . $sql . '<br>' . $ob->con->error);
			...
			if(isset($_POST['approveUser'])){
				...
				$sql = "UPDATE users SET status='1' WHERE user_id='$user_id' ";
				$result = mysqli_query($ob->con,"SELECT * FROM `users` WHERE user_id='$user_id'");
				$email = '';
				while($row = mysqli_fetch_array($result))
				{
					$email = $row['email'];
				...
				<p>This is a approval eMail, please login BidBea account by clicking this <a href="'.$ob->home_page.'/login.php" target="_blank"><b>link</b></a>.</p>
  ...
  ?>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#tbodyDataTable").on('click','#govt_id',function(){
				$("#imageModal").modal("show");
				var currentRow=$(this).closest("tr"); 
				var govtID = currentRow.find("td:eq(4)").text().trim();
			    $("#govtImg").attr("src", "assets/govt_id/"+govtID+".png");
